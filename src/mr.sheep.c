...
#define SKIP                                                                   \
    { }
#define ERR()                                                                  \
    printf("zorgin: unimplemented bytecode '0x%02x'", INST(0));                \
    return 1;
...
// CASE_X(JMB, inst_ptr -= (uint8_t), ;); // uncond. extra neg jmp
...
// TODO: jump back is buggy may be we are parsing neg number wrong, or the
// offset for backwadr jumnp needs to take into acount the size of the
// instruction...
uint8_t sheep_exec(
...
int main(int argc, char* argv[])
{
    ...
    uint8_t exit_code = sheep_exec(code, size, ram);
